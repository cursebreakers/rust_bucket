...
pub fn generate_index_html(bucket_path: &str) -> io::Result<()> {
    ...
    fn process_dir(path: &Path, depth: usize, bucket: &Path) -> io::Result<String> {
        ...
        for entry in entries.iter().filter(|entry| entry.path().is_dir()) {
            ...
            // Process directory content recursively
    ...
    let html_content = process_dir(bucket, 0, bucket)?;
